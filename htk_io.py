...
def open(f, mode=None, veclen=13):
    ...
    if mode is None:
        if hasattr(f, 'mode'):
            ...
            mode = 'rb'
    if mode in ('r', 'rb'):
        return HTKFeatReader(f) # veclen is ignored since it's in the file
    elif mode in ('w', 'wb'):
        return HTKFeatWriter(f, veclen)
    else:
        raise Exception, "mode must be 'r', 'rb', 'w', or 'wb'"

def htk_read(filename):
    reader = HTKFeatReader(filename)
    data = reader.getall()
    del reader
    return data

def htk_write(filename, veclen, data):
    writer = HTKFeatWriter(filename, veclen)
    writer.writeall(data)
    del writer
    return True

class HTKFeatReader(object):
